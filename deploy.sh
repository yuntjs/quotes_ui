#!/bin/bash
ssh -o "StrictHostKeyChecking no" docker@ec2-52-14-238-30.us-east-2.compute.amazonaws.com 'export DOCKER_USER="$DOCKER_USER" DOCKER_PASS="$DOCKER_PASS"; bash -s' <<'ENDSSH'
  sudo apk update
  sudo apk add git
  ...
  docker stack deploy -c quotes_api/docker-compose.yml app
ENDSSH
